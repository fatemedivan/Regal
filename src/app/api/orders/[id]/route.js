...
export async function GET(request, { params }) {
  try {
    ...
    if (order.userId !== userId) {
      return NextResponse.json({ message: 'شما اجازه دسترسی به این سفارش را ندارید.' }, { status: 403 });
    }
    ...
    if (error.message.includes('Authentication required') || error.message.includes('Invalid or expired token')) {
      return NextResponse.json({ message: 'برای دسترسی به جزئیات سفارش، احراز هویت لازم است.' }, { status: 401 });
    }
    return NextResponse.json({ message: 'خطای داخلی سرور در دریافت جزئیات سفارش.' }, { status: 500 });
  }
}